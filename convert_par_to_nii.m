...
function [dyn, image_meta, image_info] = convert_SingleEcho_4D(par_fullpaths, outdir)
    ...
    for i=1:numel(par_fullpaths);
        if nargout > 1
            ...
            [dyn{i}, image_meta{i}] = load_parrec(par_fullpaths{i},[],[],[],[],[]);
        end
        ...
        dyn{i}.data = fliplr(dyn{i}.data);
        dyn{i}.hdr = struct(                   ...
            'fname',   '',                     ...
            'dim',     dim(1:3),               ...
            'pinfo',   [slope,intercept,352]', ...
            'dt',      [spm_type('uint16'),1], ...
            'mat',     r.mat,                  ...
            'n',       [],                     ...
            'descrip', '',                     ...
            'private', []);
...
function [dyn, image_meta, image_info] = convert_DualEcho_4D(par_fullpaths, echo_labels, outdir)
    ...
    for i=1:numel(par_fullpaths);
        ...
        for j = 1:numel(dyn{i});
            ...
            dyn{i}(j).run = i;
            dyn{i}(j).data = (dyn{i}(j).data .* slope) + intercept;
            dyn{i}(j).data = fliplr(dyn{i}(j).data);
            dyn{i}(j).hdr = struct(                   ...
                'fname',   '',                     ...
                'dim',     dim(1:3),               ...
                'pinfo',   [slope,intercept,352]', ...
